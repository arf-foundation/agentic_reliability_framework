...
class RAGGraphMemory:
    ...
    def _generate_incident_id(self, event) -> str:
        ...
        lat = resolve_metric(event, "latency_p99") or 0.0
        err = resolve_metric(event, "error_rate") or 0.0
    ...
    def store_incident(self, event, analysis: Dict[str, Any]) -> str:
        ...
        with self._transaction():
            ...
            node = IncidentNode(
                ...
                metrics={
                    "latency_ms": resolve_metric(event, "latency_p99"),
                    "error_rate": resolve_metric(event, "error_rate"),
                    "throughput": resolve_metric(event, "throughput"),
            ...
            if len(self.incident_nodes) > MAX_INCIDENT_NODES:
                ...
                self._faiss_to_incident.pop(
                    self.incident_nodes[oldest].faiss_index, None
                )
    ...
    def find_similar(self, event, analysis: Dict[str, Any], k: int = 5) -> List[IncidentNode]:
        ...
        with self._transaction():
            if cache_key in self._similarity_cache:
                ...
                return [
                    res.incident_node for res in self._similarity_cache[cache_key]
                ]
        ...
        with self._transaction():
            ...
            self._similarity_cache[cache_key] = [
                SimilarityResult(node, 0.0, 0.0, 0) for node in results
            ]
    ...
    def get_historical_effectiveness(
        self, action: str, component_filter: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        for outcome in self.outcome_nodes.values():
            ...
            if action in outcome.actions_taken:
                ...
                if incident and (
                    ...
                    or incident.component == component_filter
                ):
        ...
        resolution_times = [
            o.resolution_time_minutes for o in relevant if o.success
        ]
    ...
    def get_most_effective_actions(
        self, component: str, k: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        for node in self.incident_nodes.values():
            component_dist[node.component] = (
                component_dist.get(node.component, 0) + 1
            )
        ...
        return {
            ...
            / (self._stats["similarity_searches"] or 1),
            "incidents_with_outcomes": len(
                set(o.incident_id for o in self.outcome_nodes.values())
            ),
            "avg_outcomes_per_incident": len(self.outcome_nodes)
            / (len(self.incident_nodes) or 1),
