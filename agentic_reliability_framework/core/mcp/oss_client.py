...
# Adjusted imports for new repo structure
...
# Try to import oss_config, fallback to dummy if not available
try:
    from agentic_reliability_framework.core.config.oss_config import oss_config
except ImportError:
...
class OSSMCPClient:
    ...
    def _validate_oss_config(self) -> None:
        ...
        # Check OSS compliance – if not compliant, warn but continue (no raise)
        if not check_oss_compliance():
            logger.warning(
                "Enterprise environment detected, but OSS client will continue in advisory mode. "
                "To use enterprise features, please use the Enterprise edition."
            )
            # No raise, just log
    ...
    async def _query_rag_for_similar_incidents(
        ...
        try:
            # SAFE IMPORT: Try multiple import strategies for RAG graph
            rag_graph = None
            
            # Try absolute import (works when installed)
            try:
                from agentic_reliability_framework.runtime.memory.rag_graph import get_rag_graph
    ...
    def _create_compatible_event_for_rag(self, component: str, context: Optional[Dict[str, Any]] = None) -> Optional[Any]:
        ...
        try:
            ...
            from agentic_reliability_framework.runtime.memory.models import EventSeverity, create_compatible_event
